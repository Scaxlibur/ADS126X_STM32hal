...
unsigned char ADS126xXferByte(uint8_t txData)
{
	...
	HAL_StatusTypeDef s = HAL_SPI_TransmitReceive_DMA( &hspi1, &txData, &rxData, 1);
	return (unsigned char) rxData;
}
unsigned ADS126xXferBytes(uint8_t* txData, uint8_t* rxData, uint16_t lenght) {
	while(hspi1.State == HAL_SPI_STATE_BUSY);
	HAL_StatusTypeDef s = HAL_SPI_TransmitReceive_DMA( &hspi1, txData, rxData, lenght);
	return (unsigned) s;
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if(GPIO_Pin == ADS126x_DRDY_Pin && ADS126x_Init_Done == 1) {
		...
		//ADS126xSetCS(0);
		HAL_SPI_TransmitReceive_DMA(&hspi1, &emptyTxBuffer[0], &ADS126x_DataBuf[0], ADS126x_DATA_BUF_SIZE);
		//ADS126xSetCS(1);
	}
}
